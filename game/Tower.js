...
export class Tower {
  ...
  update(dt, enemies, addProjectile, globalEffects = { emi: false, luciferium: false }, grid = null) {
    // 흑점 폭발 체크 (원거리 타워 무력화)
   ...
   */
  findTarget(enemies, grid = null) {
    let bestTarget = null;
    let maxDist = -1;

    // [Optimization] 그리드가 있으면 주변 격자만 탐색
    if (grid) {
      const nearbyEnemies = grid.getNearbyEntities(this.x, this.y, this.currentRange || this.range);
